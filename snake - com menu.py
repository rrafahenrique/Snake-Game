...
#Atualiza o arquivo txt de pontuação com pontuação alta
def atualiza_recorde(novo_ponto):
    pontos = recorde()

    with open("pontos.txt", 'w') as file:
        if novo_ponto > pontos:
            file.write(str(novo_ponto))
        else:
            file.write(str(pontos))

#Obtém a pontuação mais alta do arquivo
def recorde():
    with open("pontos.txt", 'r') as file:
        linhas = file.readlines()         #lê todas as linhas e coloca em uma lista
        pontos = int(linhas[0].strip())    #remove o \n
...
def main():
    ...
    while True:
        ...
        if cobra.colliderect(comida):
            ...
            #Chama a função para atualizar os recordes
            atualiza_recorde(pontos)
            #Condição do recorde
            if pontuação < pontos:
                pontuação = pontos
        ...
        if any (corpo == cobra_cabeca for corpo in corpo_cobra[:-4]):
            ...
            while game_over:
                ...
                texto_jogo("Recorde: " + str(pontuação), BRANCO, 30, LARGURA//2, ALTURA//2 + 150)
...
def menu():
    while True:
        JANELA.fill(PRETO)
        texto_jogo("SNAKE GAME", BRANCO, 40, LARGURA//2, ALTURA//2 - 160)

        texto_jogo("Press ENTER to Play", BRANCO, 30, LARGURA//2, ALTURA//2)
